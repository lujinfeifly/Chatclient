...
//								}
    ...
    public void writeBytes(byte[] data) {
        ...
        System.out.println("SelectorGuardHandler writeBytes to xc !!!!!!");
	...
	public void sendQueryList(String pno, String rpp, String uType ) {
		try {
			PageUtil.pageSize = Integer.parseInt(rpp);
			this.writeBytes(encodePacket(1, 6,new QueryListBean(pno, rpp, uType)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @param uid
	 */
	public void sendQueryUserInfo(String uid) {
		try {
			this.writeBytes(encodePacket(4, 2,new UserInfoBean(uid)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
